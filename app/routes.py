...
# Função para editar ação em específico
@app.route("/editar_acoes_especifica/<ticker>", methods=["PUT"])
def editar_acao_especifica(ticker):
    session = Session()

    acao = session.get(AnaliseAcoes, ticker.upper())

    if acao is None:
        session.close()
        return {"erro": "Ação não encontrada"}, 404

    dados = request.get_json()
    acao.analise = dados["analise"]
    acao.data_analise = datetime.now()

    session.commit()

    resultado = {
        "acao": acao.acao,
        "analise": acao.analise,
        "data_analise": acao.data_analise.isoformat()
    }
